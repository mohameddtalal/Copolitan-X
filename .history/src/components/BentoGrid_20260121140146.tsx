...
const BentoGrid = () => {
    ...
    const pages = [
        ...
        // Page 3
        [
            { id: 16, title: "Data Hub", description: "A streamlined repository for all data collect, analyze, and access key metrics to drive informed decisions.", color: "bg-[#242424]", span: "col-span-1 row-span-1" },
            { id: 2, title: "", description: "", color: "bg-[var(--background)]", span: "col-span-1 row-span-1", image: "/cards/CIRCULAR TEXT.svg" },
            { id: 4, title: "fatma Ashraf", description: "Co-founder", color: "bg-[var(--background)]", span: "col-span-1 row-span-2", isSplit: true, profileImage: "/cards/profile.svg", characterImage: "/cards/character.svg" },
                        ...
                        <div key={pageIndex} className="min-w-full h-full px-1.5 sm:px-2 md:px-2 lg:px-3 xl:px-4 flex items-center justify-center">
                                            ...
                                            <div className="flex flex-col h-full gap-2 sm:gap-3 lg:gap-4 sm:gap-3 ">
                                                ...
                                                <div className="flex flex-row items-center gap-2 sm:gap-3 p-1.5 sm:p-2 md:p-2.5 lg:p-3 xl:p-4 bg-[#7029CF] rounded-xl sm:rounded-2xl lg:rounded-3xl">
                                                    ...
                                                    <div className="flex flex-col justify-center flex-1">
                                                        <h2 className="text-[clamp(14px,2.5vw,24px)] leading-[clamp(22px,3vw,30px)] font-semibold italic font-lora">{card.title}</h2>
                                                        <p className="text-[clamp(12px,1.8vw,16px)] leading-[clamp(18px,2.5vw,24px)] opacity-90 font-gtwalsheim leading-[16px]">{card.description}</p>
                                                    </div>
                                                </div>
                            ...
                            />
                                                ...
                                                <div className="relative z-10 mt-auto">
                                                    <h2 className="text-[clamp(24px,5vw,48px)] leading-[clamp(36px,7vw,60px)] mb-1 md:mb-1.5 lg:mb-2 xl:mb-3  font-lora ">{card.title}</h2>
                                                    <p className="text-[clamp(12px,2.5vw,16px)] leading-[clamp(18px,3.5vw,24px)] opacity-90 line-clamp-2 font-gtwalsheim">{card.description}</p>
                                                </div>
                                            </>
                                        ) : card.cardImage ? (
                                            ...
                                            <>
                                                {card.layoutType === 'column' ? (
                                                    ...
                                                    <div className="flex flex-col gap-1 sm:gap-1 md:gap-2 lg:gap-3 xl:gap-4 flex-1">
                                                        <h2 className={`text-[24px] sm:text-xs md:text-sm lg:text-base xl:text-xl  font-lora ${card.id === 8 ? 'text-black' : 'text-white'}`}>{card.title}</h2>
                                                        ...
                                                        <p className={`text-[12px] sm:text-[9px] md:text-xs lg:text-xs xl:text-sm opacity-90 line-clamp-3 font-gtwalsheim leading-[16px] ${card.id === 8 ? 'text-black' : 'text-white'}`}>{card.description}</p>
                                                    </div>
                                                ) : (
                                                    ...
                                                    <div className="flex flex-row gap-1 sm:gap-1.5 md:gap-2 lg:gap-2.5 xl:gap-3 items-start h-full">
                                                        <div className="flex flex-col flex-1 gap-0.5 sm:gap-0.5 md:gap-1 lg:gap-1.5 xl:gap-2">
                                                            <h2 className={`text-[clamp(14px,2.5vw,24px)] leading-[clamp(22px,3vw,30px)] font-semibold italic font-lora ${card.id === 8 ? 'text-black' : 'text-white'}`}>{card.title}</h2>
                                                            <p className={`text-[clamp(8px,2vw,12px)] leading-[clamp(14px,2.5vw,20px)] opacity-90 font-gtwalsheim ${card.id === 8 ? 'text-black' : 'text-white'}`}>{card.description}</p>
                                                        </div>
                                                        <img
                                                            src={card.cardImage}
                                                            alt={card.title}
                                            ...
                                            <>
                                                <div>
                                                    <h2 className={`text-[clamp(14px,2.5vw,24px)] leading-[clamp(22px,3vw,30px)] font-semibold mb-0.5 sm:mb-1 md:mb-1.5 lg:mb-2 xl:mb-3 italic font-lora ${card.id === 8 || card.id === 1 || card.id === 9 ? 'text-black' : 'text-white'}`}>{card.title}</h2>
                                                    <p className={`text-[clamp(12px,1.8vw,16px)] leading-[clamp(18px,2.5vw,24px)] opacity-90 line-clamp-3 font-gtwalsheim leading-[16px] ${card.id === 8 || card.id === 1 || card.id === 9 ? 'text-black' : 'text-white'}`}>{card.description}</p>
                                                </div>
