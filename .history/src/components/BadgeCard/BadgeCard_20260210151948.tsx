...
export default function BadgeCard({ card }: any) {
  ...
  const getTitleSizeClass = (id: number) => {
    ...
    // if ([ ].includes(id)) return "title-md";
  ...
  const getDescSizeClass = (id: number) => {
    ...
    if ([12,13].includes(id)) return "desc-md";
  ...
  if (card.flippable) {
    return (
      <div className={`${positionClasses} group perspective-1000 relative rounded-xl sm:rounded-2xl lg:rounded-3xl h-full cursor-pointer`}
        ...
        <div className={`flip-wrapper w-full h-full relative preserve-3d transition-transform duration-1000 ease-in-out ${isFlipped ? 'rotate-y-180' : ''}`}>
