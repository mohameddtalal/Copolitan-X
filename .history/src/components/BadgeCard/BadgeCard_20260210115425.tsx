...
export default function BadgeCard({ card }: any) {
        ...
        <div className="flex flex-col h-full gap-2 sm:gap-3 lg:gap-4 sm:gap-3 ">
          ...
          <div className="flex flex-row items-center gap-2 sm:gap-3 p-1.5 sm:p-2 md:p-2.5 lg:p-3 xl:p-4 bg-[#7029CF] rounded-xl sm:rounded-2xl lg:rounded-3xl">
            ...
            <div className="flex flex-col justify-center flex-1">
              ...
              <p
                className={`font-gtwalsheim whitespace-pre-line ${getDescSizeClass(card.id)} scrollable-desc`}
              >
                {card.description}
              </p>
            </div>
      ...
      return (
        <>
          ...
          <div className="relative z-10 mt-auto">
            ...
            <p
              className={` line-clamp-2 font-gtwalsheim whitespace-pre-line ${getDescSizeClass(card.id)} scrollable-desc`}
            >
              {card.description}
            </p>
          </div>
        ...
        <>
          {card.layoutType === "column" ? (
            ...
            <div className="flex flex-col gap-1 sm:gap-1 md:gap-2 lg:gap-3 xl:gap-4 flex-1">
              ...
              <p
                className={` font-gtwalsheim whitespace-pre-line ${getDescSizeClass(card.id)} ${card.id === 8 ? "text-black" : "text-white"} scrollable-desc`}
              >
                {card.description}
              </p>
            </div>
    ...
    return (
      <>
        <div className={styles[`card-${card.id}`]}>
          ...
          <p
            className={` font-gtwalsheim whitespace-pre-line ${getDescSizeClass(card.id)} ${card.id === 8 || card.id === 1 || card.id === 9 || card.id === 15 ? "text-black" : "text-white"} scrollable-desc`}
          >
            {card.description}
          </p>
        </div>
  ...
  const positionClasses = card.id === 4 ? "col-start-4 row-start-1 col-span-1 row-span-2" : card.span;
  ...
  if (card.flippable) {
    return (
      <div className={`${positionClasses} group perspective-1000 relative`}>
        <div className="flip-wrapper w-full h-full relative preserve-3d transition-transform duration-300">
