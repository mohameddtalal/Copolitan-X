...
const BentoGrid = () => {
    ...
    const pages = [
        ...
        // Page 2
        [
            { id: 9, title: "Partnerships <br> Network", description: "Oversee and manage all partnerships, deals, and strategic collaborations", color: "bg-[#FFD3D2]", span: "col-span-1 row-span-1" },
            { id: 2, title: "", description: "", color: "bg-[var(--background)]", span: "col-span-1 row-span-1", image: "/cards/CIRCULAR TEXT.svg" },
                                        ...
                                        {card.isSplit ? (
                                            ...
                                            <div className="flex flex-col h-full gap-2 sm:gap-3 lg:gap-4 sm:gap-3 ">
                                                ...
                                                <div className="flex flex-row items-center gap-2 sm:gap-3 p-1.5 sm:p-2 md:p-2.5 lg:p-3 xl:p-4 bg-[#7029CF] rounded-xl sm:rounded-2xl lg:rounded-3xl">
                                                    ...
                                                    <div className="flex flex-col justify-center flex-1">
                                                        <h2 className={`lora-semibold-italic ${getTitleSizeClass(card.id)}`}>{card.title}</h2>
                                                        <p className={`opacity-90 font-gtwalsheim ${getDescSizeClass(card.id)}`}>{card.description}</p>
                                                    </div>
                                                </div>
                                            ...
                                            <>
                                                ...
                                                <div className="relative z-10 mt-auto">
                                                    <h2 className={`mb-1 md:mb-1.5 lg:mb-2 xl:mb-3 lora-semibold-italic ${getTitleSizeClass(card.id)}`}>{card.title}</h2>
                                                    <p className={`opacity-90 line-clamp-2 font-gtwalsheim ${getDescSizeClass(card.id)}`}>{card.description}</p>
                                                </div>
                                            </>
                                        ) : card.cardImage ? (
                                            ...
                                            <>
                                                {card.layoutType === 'column' ? (
                                                    ...
                                                    <div className="flex flex-col gap-1 sm:gap-1 md:gap-2 lg:gap-3 xl:gap-4 flex-1">
                                                        <h2 className={`lora-semibold-italic ${getTitleSizeClass(card.id)} ${card.id === 8 ? 'text-black' : 'text-white'}`}>{card.title}</h2>
                                                        ...
                                                        <p className={`opacity-90 line-clamp-3 font-gtwalsheim ${getDescSizeClass(card.id)} ${card.id === 8 ? 'text-black' : 'text-white'}`}>{card.description}</p>
                                                    </div>
                                                ) : (
                                                    ...
                                                    <div className="flex flex-row gap-1 sm:gap-1.5 md:gap-2 lg:gap-2.5 xl:gap-3 items-start h-full">
                                                        <div className="flex flex-col flex-1 gap-0.5 sm:gap-0.5 md:gap-1 lg:gap-1.5 xl:gap-2">
                                                            <h2 className={`lora-semibold-italic ${getTitleSizeClass(card.id)} ${card.id === 8 ? 'text-black' : 'text-white'}`}>{card.title}</h2>
                                                            <p className={`opacity-90 font-gtwalsheim ${getDescSizeClass(card.id)} ${card.id === 8 ? 'text-black' : 'text-white'}`}>{card.description}</p>
                                                        </div>
                                                        <img
                                                            src={card.cardImage}
                                                            alt={card.title}
                                            ...
                                            <>
                                                <div>
                                                    <h2 className={`mb-0.5 sm:mb-1 md:mb-1.5 lg:mb-2 xl:mb-3 lora-semibold-italic ${getTitleSizeClass(card.id)} ${card.id === 8 || card.id === 1 || card.id === 9 || card.id === 16 ? 'text-black' : 'text-white'}`}>{card.title}</h2>
                                                    <p className={`opacity-90 line-clamp-3 font-gtwalsheim ${getDescSizeClass(card.id)} ${card.id === 8 || card.id === 1 || card.id === 9 || card.id === 16 ? 'text-black' : 'text-white'}`}>{card.description}</p>
                                                </div>
