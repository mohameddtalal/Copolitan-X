            ...
            <div key={pageIndex} className="min-w-full h-full px-1.5 sm:px-2 md:px-2 lg:px-3 xl:px-4 flex items-center justify-center">
              <div className="grid grid-cols-4 grid-rows-3 gap-2 sm:gap-3 lg:gap-4 w-full h-full auto-rows-fr">
                {page.map((card) => (
                  <div
  key={card.id}
  className={`${card.color} ${card.isSplit ? 'col-start-4 row-start-1 col-span-1 row-span-2' : card.span} rounded-xl sm:rounded-2xl lg:rounded-3xl ${!card.isSplit ? 'p-2 sm:p-2.5 md:p-3 lg:p-4 xl:p-6' : ''} flex flex-col justify-between text-white shadow-xl transition-transform cursor-pointer group overflow-hidden relative`}
  style={card.isSplit ? {} : {gridColumn: 'auto', gridRow: 'auto'}}
>
                    {card.isSplit ? (
